...
const EditarPregunta = forwardRef(({ id, title, modo }, ref) => {    
    ...
    useEffect(() => {
        if (modo === "editar" && id) {
            ...
            let pregunta = preguntas.listado?.[id];
    
            // Buscar en editadas si no está en listado
            if (!pregunta) {
                pregunta = preguntas.editadas.find(p => p.id === id);
            }
    
            // Buscar en nuevas si tampoco está en editadas
            if (!pregunta) {
                pregunta = preguntas.nuevas.find(p => p.id === id);
            }
