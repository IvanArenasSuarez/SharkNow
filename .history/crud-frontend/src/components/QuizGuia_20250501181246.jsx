...
export default function QuizGuia() {
    ...
    const [sessionData, setSessionData] = useState(null);  // Para guardar la sesión en formato JSON
    ...
    const handleAnswer = (selectedOption) => {
        ...
        setQuestionTime(0);
        setQualityPerQuestion([...qualityPerQuestion, { questionId: questions[step].id, quality }]);
    ...
    const nextStep = () => {
        ...
        if (step < questions.length - 1) {
            ...
            const sessionInfo = {
                ...
                qualityPerQuestion: qualityPerQuestion, // Calidad por pregunta
                score: score, // Puntaje
                totalTime: timer, // Tiempo total
            };
            setSessionData(sessionInfo);  // Guardamos la sesión
            console.log(JSON.stringify(sessionInfo, null, 2));  // Muestra la sesión como JSON
        }
    };
                        ...
                        <ul className="list-disc pl-5">
                            {qualityPerQuestion.map((quality, index) => (
                                <li key={index} className="text-gray-300">
                                    Pregunta {index + 1}: Calidad {quality.quality}
                                </li>
                            ))}
                        </ul>
                    </div>
