...
export default function QuizGuia() {
    ...
    const checkMatchingAnswers = () => {
        ...
        correctPairs.forEach((pair, leftIndex) => {
            // Comprobar si el valor de la columna derecha coincide con el valor correcto de la columna izquierda
            const assignedRightValue = matchingAnswers[leftIndex];  // El valor asignado por el usuario
            ...
            {!completed ? (
                ...
                <div className="text-center">
                    ...
                    <div className="mt-4 text-left">
                        <h3 className="text-md font-bold text-yellow-400">Tiempo por pregunta:</h3>
                        <ul className="list-disc pl-5">
                            {questionTimes.map((time, index) => (
                                <li key={index} className="text-gray-300">Pregunta {index + 1}: {time}s</li>
                            ))}
                        </ul>
                    </div>
                </div>
