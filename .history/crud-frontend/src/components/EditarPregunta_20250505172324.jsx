...
const EditarPregunta = forwardRef(({ id, title, modo, preguntaData = null }, ref) => {
    ...
    const guardarPreguntaEditada = () => {
        ...
        // 1. Eliminar de "listado" si existe
        preguntas.listado = preguntas.listado.filter(p => p.id !== preguntaEditada.id);
    
        // 2. Reemplazar o agregar en "editadas"
        const index = preguntas.editadas.findIndex(p => p.id === preguntaEditada.id);
        if (index !== -1) {
            preguntas.editadas[index] = preguntaEditada; // Reemplazar
        } else {
            preguntas.editadas.push(preguntaEditada); // Agregar
        }
