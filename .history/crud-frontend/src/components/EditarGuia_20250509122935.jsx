...
export default function EditarGuia() {
  ...
  const actualizarListadoPreguntas = () => {
  const nuevasPreguntas = JSON.parse(localStorage.getItem("preguntas")) || {};
  const listado = [
    ...(nuevasPreguntas.listado || []),
    ...(nuevasPreguntas.nuevas || []),
    ...(nuevasPreguntas.editadas || []),
  ].filter(p => !(nuevasPreguntas.eliminadas || []).some(e => e.id === p.id));
        ...
        <div className="w-full lg:w-1/2">
          <ul className="bg-base-100 rounded-box shadow-md h-[800px] overflow-y-auto">
            ...
            <button className="btn btn-outline btn-primary w-3/4 h-12" onClick={abrirModalCrear}>+ Añadir Pregunta</button>
              <EditarPregunta onChange={actualizarListadoPreguntas}
