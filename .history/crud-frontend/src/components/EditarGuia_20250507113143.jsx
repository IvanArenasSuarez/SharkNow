...
export default function EditarGuia() {
  ...
  useEffect(() => {
    ...
    if (!preguntas) {
        ...
        preguntas = initialData;  // Asignar el valor inicial si no existe
    } else {
        preguntas = JSON.parse(preguntas);  // Convertir de JSON a objeto
    }
    console.log(JSON.stringify(preguntas, null, 2));  // Ahora imprimimos el objeto correctamente
}, []);
  ...
  const abrirModalCrear = () => {
    ...
    refEditarPregunta.current?.openModal(); // Asegúrate de que el método openModal esté disponible
  }  
    ...
    const handleEliminarPregunta = (index) => {
      ...
      const preguntasStorage = JSON.parse(localStorage.getItem("preguntas"));
      const todas = [...preguntasStorage.listado, ...preguntasStorage.nuevas, ...preguntasStorage.editadas];
    
      const preguntaEliminada = todas[index];
    
      // Guardar en eliminadas
      preguntasStorage.eliminadas.push(preguntaEliminada);
    
      // Eliminar de su categoría original
      ["listado", "nuevas", "editadas"].forEach(categoria => {
        preguntasStorage[categoria] = preguntasStorage[categoria].filter(p => p !== preguntaEliminada);
      });
    
      // Actualizar localStorage y estado
      localStorage.setItem("preguntas", JSON.stringify(preguntasStorage));
      setPreguntas([...preguntasStorage.listado, ...preguntasStorage.nuevas, ...preguntasStorage.editadas]);
    };
            ...
            {preguntas.map((pregunta, index) => (
              <li key={index} className="flex items-center gap-4 px-3 border-b py-2">
                <div className="flex flex-col flex-grow">
                  <div className="font-semibold text-lg">{pregunta.type}</div>
            ...
            <button className="btn btn-outline btn-primary w-3/4 h-12" onClick={abrirModalCrear}>+ Añadir Pregunta</button>
              <EditarPregunta
                  ...
                  onSave={(nuevaPregunta) => {
                    const nuevas = [...preguntas, nuevaPregunta];
                    setPreguntas(nuevas);
