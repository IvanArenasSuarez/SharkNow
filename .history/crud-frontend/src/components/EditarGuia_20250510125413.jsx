...
export default function EditarGuia() {
  ...
  useEffect(() => {
    async function obtenerPreguntas() {
      try {
        const response = await fetch(`http://localhost:4000/guias/preguntas?id_guia=${guias.id}`); 
...
useEffect(() => {
  return () => {
    localStorage.removeItem("preguntas");
    console.log("Se eliminó preguntas de localStorage al salir del componente");
  };
}, []);
