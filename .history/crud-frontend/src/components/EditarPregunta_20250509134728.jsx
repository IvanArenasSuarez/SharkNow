...
const EditarPregunta = forwardRef(({ id, title, modo, onSave }, ref) => {    
    ...
    // Reemplazar en listado
    preguntas.listado[idFinal] = preguntaEditada;

    // Buscar si está en "nuevas"
    const indexNueva = preguntas.nuevas.findIndex(p => p.id === idFinal);
    if (indexNueva !== -1) {
        preguntas.nuevas[indexNueva] = preguntaEditada;
    } else {
        // Si no está en "nuevas", manejar como "editada"
        const indexEditada = preguntas.editadas.findIndex(p => p.id === idFinal);
        if (indexEditada !== -1) {
            preguntas.editadas[indexEditada] = preguntaEditada;
        } else {
    ...
    const inicializarOpciones = (type) => {
        switch (type) {
            ...
            case "matching":
                return Array(3).fill().map(() => ({ izquierda: "", derecha: "" }));
            default:
                return [];
        }
    };
