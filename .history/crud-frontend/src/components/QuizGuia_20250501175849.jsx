...
const buildSessionData = () => {
    const respuestas = qualityPerQuestion.map((quality, index) => ({
        id_reactivo: questions[index].id,
        calidad: quality
    }));
...
export default function QuizGuia() {
    ...
    useEffect(() => {
        if (completed) {
            ...
            // Aquí puedes guardar sessionData en tu backend o en un estado global
    ...
    useEffect(() => {
        if (completed && !horaFin) {
            const now = new Date();
            setHoraFin(now.toISOString());
        }
    }, [completed]);
    ...
    const handleAnswer = (selectedOption) => {
        ...
        setQuestionTime(0);
        setQualityPerQuestion((prev) => [...prev, quality]);
        setQualityLog((prev) => ({
            ...prev,
            [questions[step].id]: quality
        }));
